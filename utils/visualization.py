...
def show_from_center(objs):
    ...
    # vis.run()
    # vis.destroy_window()
    save_visualization_as_image(geometries, "output_center.png")
...
def show_from_side(objs):
    ...
    # vis.run()
    # vis.destroy_window()
    save_visualization_as_image(geometries, "output_side.png")

def save_visualization_as_image(geometries, filename="output.png"):
    vis = o3d.visualization.Visualizer()
    vis.create_window(visible=False)  # Disable GUI window
    [vis.add_geometry(geometry) for geometry in geometries]
    vis.poll_events()
    vis.update_renderer()
